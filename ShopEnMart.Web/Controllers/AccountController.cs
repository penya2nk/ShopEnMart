    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult _Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = _unitOfWork.GetRepositoryInstance<Members>().GetFirstOrDefaultByParameter(i => i.EmailId == model.UserEmailId && i.Password == EncryptedPassword && i.IsDelete == false);
        ...
        public ActionResult Register(RegisterViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                mem.IsActive = true;
                mem.IsDelete = false;
                _unitOfWork.GetRepositoryInstance<Members>().Add(mem);
        ...
        public JsonResult CheckEmailExist(string UserEmailId)
        {
            ...
            var EmailExist = _unitOfWork.GetRepositoryInstance<Members>().GetFirstOrDefaultByParameter(i => i.MemberId != LoginMemberId && i.EmailId == UserEmailId && i.IsDelete == false);
        ...
        {
            if (_unitOfWork.GetRepositoryInstance<Members>().GetFirstOrDefaultByParameter(i => i.EmailId == fpm.EmailId && i.IsActive == true && i.IsDelete == false) != null)
            {
        ...
        {
            var ExistingDetails = _unitOfWork.GetRepositoryInstance<Members>().GetFirstOrDefaultByParameter(i => i.EmailId == rpm.EmailId && i.IsActive == true && i.IsDelete == false);
            if (ExistingDetails != null)
            {
                ExistingDetails.Password = EncryptDecrypt.Encrypt(rpm.NewPassword, true);
                ExistingDetails.ModifiedOn = DateTime.UtcNow;
                _unitOfWork.GetRepositoryInstance<Members>().Update(ExistingDetails);
